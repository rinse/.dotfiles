...
採用した構成は Xephyr によるネストである(Xephyr 自体の説明と必要性の理屈は [Xephyr とは何か](/xephyr.md) にまとめた)。
...
# リサイズ後に下部が見切れたら再描画漏れを疑う

ホスト側で Xephyr ウィンドウを最大化・リサイズした後、内部デスクトップの下部が欠けて見えることがある。
このとき X サーバー内部のフレームバッファは正常なことがある（`DISPLAY=:1 import -window root shot.png` で内部を直接撮って確認できる。ジオメトリも `xwininfo` / `xrandr` で突き合わせる）。
内部が正常で表示だけ欠けているなら、原因はレイアウトではなく WSLg への再描画（damage）伝搬の漏れで、`DISPLAY=:1 xrefresh` の強制再描画で解消する。
xmonad と xmobar 自体は RandR のスクリーンサイズ変更に追従する（実測済み。Xephyr `-resizeable` はホストウィンドウのサイズ変更を内部スクリーンに反映する）。
